...
// Define the list of sponsors
const sponsors = [
  { username: 'sponsor1', name: 'Sponsor 1' },
  { username: 'sponsor2', name: 'Sponsor 2' },
  // Add more sponsors here
  // Example: { username: 'channel_username', name: 'Channel Name' }
];

// Define a function to check if a user is subscribed to all sponsors
async function isUserSubscribed(ctx) {
  for (const sponsor of sponsors) {
    try {
      const member = await bot.telegram.getChatMember(sponsor.username, ctx.from.id);
...
// Define a function to send the list of sponsors to a user
async function sendSponsorsList(ctx) {
  const sponsorList = sponsors.map((sponsor, index) => `\n${index + 1}. t.me/${sponsor.username} - ${sponsor.name}`).join('');
  await ctx.reply(`To use the bot, please subscribe to our sponsors:${sponsorList}`);
}

// Define a middleware to check if a user is subscribed to all sponsors
async function checkSubscription(ctx, next) {
  if (!isBotRunning) {
    return;
  }
  if (!await isUserSubscribed(ctx)) {
    await sendSponsorsList(ctx);
    return;
  }
  await next();
}
